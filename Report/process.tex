...
For this purpose, we used cross-compiling toolchains to build executables compatible with the ARM processors which resides in the TV. However, we failed
...
Our solution was to emulate the ARMv7 processor with QEMU. Within this emulation, we installed a copy of the Linux Ubuntu OS, in which we were able to install the Python package with the apt tool.
To achieve this, it was also needed to enable networking within QEMU. Then we copied the binary files to the TV, after which we could succesfully run the Python interpreter together with all its dependencies.
