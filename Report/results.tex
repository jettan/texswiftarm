...
Figure \ref{graph:idle} shows the CPU and memory usage of only the application, while being idle.
The peak in the beginning depicts the initialisation of the application, where modules such as Dispersy are start up.
Figure \ref{graph:500kbs} shows the CPU and memory usage while streaming the video at 500 kb/s. It can be seen that the CPU usage is
always under 16\%, while this upper limit grows with the bitrate we set, as shown in \ref{700kbs) and \ref{1Mb/s}.
...
\clearpage
\subsection{Total Memory/CPU usage}
